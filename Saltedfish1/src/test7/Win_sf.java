	...
	//为了解决shell不能被继承的问题
		...
		//分割主窗体
		 protected void initGui() 
		 {
			...
			/* //尝试在这里启动进度条更新线程
			// BarUpdater barUpdater = new BarUpdater(display,scale2,this);
			// barUpdater.setDaemon(true);//指明这个线程是一个守护线程，当除守护线程外的所有线程关闭时JVM也会关闭！！
	         // barUpdater.start();
			 //尝试改用vlcjMP内置的事件机制
			 //List list_zm=new List(labels[1],SWT.SINGLE | SWT.H_SCROLL|SWT.BORDER|SWT.WRAP);//水平滚动，纵列显示
			 //list_zm.setBounds(10, 100, 600, 300);*/
			 
			 //纵向的字幕列显示容器
			 this. comp_zm0=new ScrolledComposite (labels[1],SWT.BORDER|SWT.H_SCROLL);
			 comp_zm0.setBounds(10, 100, 600, 260);
			 this.comp_zm=new Composite(comp_zm0,SWT.NONE|SWT.H_SCROLL);
			 comp_zm.setBounds(0, 0, 60000, 260);
			 comp_zm0.setContent(comp_zm);			 
			 //String[] arr_zm=Util_File.ReadTxtzm();//一行一行的读入txt文件，作为list的元素加入列表
			 //setCompContents(comp_zm,arr_zm);
			 Button btn_read=new Button(labels[1],SWT.PUSH);
			 btn_read.setBounds(620, 100, 70, 20);
			 btn_read.setText("读文本");
			 Event_cb.registerCallback(btn_read,this,"ReadTxt");
			 Button btn_srt=new Button(labels[1],SWT.PUSH);
			 btn_srt.setBounds(620, 130, 70, 20);
			 btn_srt.setText("srt");
			 Event_cb.registerCallback(btn_srt,this,"ExportSrt");
			 Button btn_wav=new Button(labels[1],SWT.PUSH);
			 btn_wav.setBounds(620, 160, 70, 20);
			 btn_wav.setText("wav");
			 Event_cb.registerCallback(btn_wav,this,"ExportWav");
			 Button btn_combine=new Button(labels[1],SWT.PUSH);
			 btn_combine.setBounds(620, 190, 70, 20);
			 ...
			 //-start
			 //打开已有项目
			 List list_xm=new List(labels[2], SWT.SINGLE | SWT.V_SCROLL|SWT.BORDER);		
		 ...
		 public void PlayFile()
		 {
			 try
			 {
				 ...
				 {//如果字幕列表里有btn_sf在，需要找到当前同步位置
					 int_col_index=len-1;
					 int flag_set=0;
					 for(int i=0;i<len;i++)
					 {//这个是按添加顺序排序的吗？						
							 Btn_sf btn_col=(Btn_sf)(arr_zm[i]);
							 if(btn_col.int_starttime>Integer.valueOf(this.lab_currenttime.getText()))
							 {//如果能找到紧挨着的下一个btn就使用下一个，否则寻找第一个没有使用过的btn
					 ...
					 if(flag_set==0)
					 {
						 ...
						 {
							 Btn_sf btn_col=(Btn_sf)(arr_zm[i]);
							 if(btn_col.int_endtime==0)//表示这一列字幕还没有用TTS生成过
							 {
								 int int_x=btn_col.int_colindex*30;							 
								 comp_zm.setLocation(-int_x, 0);
								 comp_zm0.getHorizontalBar().setSelection(int_x);
					 ...
					 btn_lastcol=(Btn_sf)(arr_zm[int_col_index]);//把上一个获取焦点的按钮标记下来
		 ...
		 public  void ExportSrt()
		 {
			 ...
			 if(len>0)
			 {
				...
				Util_File.saveToSrt(arr_zm,file,this);	 
			 }
			 else
			 {
