...
public class MSTTSSpeech1 {
  ...
  //完全停止播放并尽力释放资源
    ...
    public void saveToWav(Control[] arr_zm,String filePath)  
    {  //这里要区分阅读和保存时不同的com设置！！        
        // 调用输出文件流对象的打开方法，创建一个.wav文件  
        Dispatch.call(spFileStream,"Open",new Variant(filePath+".wav"),new Variant(3),new Variant(true));  
        ...
        int len=arr_zm.length;
		 int i=0;
		 Btn_sf btn_col=(Btn_sf)(arr_zm[0]);
		 while(i<len)
		 {
			 long l_currenttime=System.currentTimeMillis();
			 long l_temp=l_currenttime-l_starttime;//从开始朗读经过的毫秒数
			 //为了防止这里的调度语句执行时间积累过多，影响同步，在走马灯播放时每两句之间插入了一个100ms的延时
			 if(btn_col.int_starttime>(int)(l_temp/1000)&&btn_col.int_endtime!=0)
			 {
				 window1.lab_comment.setText("共"+len+"句，正在保存第"+i+"句");
				 System.out.println("共"+len+"句，正在保存第"+i+"句");
    ...
    public void saveToWav2(Control[] arr_zm,String filePath,int type)
    ...
    {  //这里要区分阅读和保存时不同的com设置！！        
        ...
        try 
        {
		     ...
		     for(int i=0;i<len;i++)
	        {
	        	...
	        	int int_starttime=btn_col.int_starttime;
	        	int int_endtime=btn_col.int_endtime;
	        	if(int_endtime==0)
	        	{
	        		continue;
	        	}
	        	...
	        	window1.lab_comment.setText("共"+len+"句，正在整理第"+(i+1)+"句"+System.currentTimeMillis());
				 ...
				 Dispatch.call(spVoice,"Speak",new Variant(btn_col.str_zm));  //这一句会引起崩溃
				 // 关闭输出文件流对象，释放资源  
			        Dispatch.call(spFileStream,"Close");  
			     ...
			     String cmd_amix=str_ffpath+"ffmpeg -y -i "+str_ffpath+"temp1.wav  -i "+str_ffpath
			    		 +"tempttsl.wav -filter_complex amix=inputs=2:duration=shortest:dropout_transition=0 "+str_ffpath+"temp1ttsl.wav";
