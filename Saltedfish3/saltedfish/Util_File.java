...
{
	public static String str_ffmpegpath="C:/Users/ASUS/Documents/ShareX/Tools/";//"d:"+File.separator+"SaltedFish"+File.separator;
	...
	//根据字幕数组和文件句柄保存字幕文本
	public static void saveToSrt(ArrayList<Obj_sf> list_sf,File file,final Win_sf window1)  
    {
		try {
			file.createNewFile();
			...
			for(int i=0;i<len;i++)
			 {//对于每一列字幕
				 ...
				 if(obj_col.int_alength>0&&obj_col.int_endtime>0)
				 {	
					 bw.write(String.valueOf(obj_col.int_colindex)+"\r\n");
					 bw.write(Util_File.S2str(obj_col.int_starttime)+"-->"+Util_File.S2str(obj_col.int_endtime)+"\r\n");
					 bw.write(obj_col.str_zm+"\r\n\r\n");
				     bw.flush();
				 }
			 }
			bw.close();
