...
import saltedfish.MSTTSSpeech1.ThreadDemo;
    ...
    public class ThreadDemo extends Thread
    {
       ...
       {       
    	 //通过当前的进度条时间来获取时间参照
		   ...
		   {//这是非阻塞的，但并不能保证一定先于后面的代码执行完毕
			   public void run()
			   {
				   window1.lab_video_zm.setText(text);
				   //nt_starttime=window1.scale2.getSelection();
			   }
		   });*/
		   window1.display.getDefault().syncExec(new Runnable()
    	   {//这是获取了主线程的操作权？其阻塞性导致主线程停顿！！
				        		    public void run() 
				        		    {
				        		    	//window1.lab_video_zm.setText(text);//这个实时字幕提示其实可以不要
		        ...
		        window1.display.getDefault().syncExec(new Runnable()
		    	   {//这是获取了主线程的操作权？其持续性导致主线程无法工作！！
						        		    public void run() 
						        		    {
						        		    	window1.lab_comment.setText("可以继续插入");      
						        		    	//int_starttime=window1.scale2.getSelection();
						        		    }
					});				
